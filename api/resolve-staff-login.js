...
// enumerar nombres de usuario/hoteles.
export default async function handler(req, res) {
  ...
  if (!username) return res.status(200).json({ syntheticEmail: null });

  try {
    const { data, error } = await supabase.rpc('resolve_staff_login_username', { p_username: username });
