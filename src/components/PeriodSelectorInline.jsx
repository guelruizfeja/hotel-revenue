...
export function PeriodSelectorInline({ mes, anio, onChange, aniosDisponibles, allowFuture = false }) {
  ...
  const btnFlecha = (activo) => ({ background:"none", border:`1px solid ${activo?C.border:"transparent"}`, borderRadius:6, width:26, height:26, cursor:activo?"pointer":"default", color:activo?C.textMid:C.border, fontSize:15, display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0 });

  return (
    <div style={{ userSelect:"none" }}>
      <div style={{ display:"flex", alignItems:"center", justifyContent:"center", gap:10, marginBottom:8 }}>
        <button onClick={anioAnterior} style={btnFlecha(puedeAnterior)}>‹</button>
        <p style={{ fontSize:15, fontWeight:700, color:C.text, fontFamily:"'Plus Jakarta Sans',sans-serif", minWidth:40, textAlign:"center" }}>{anio}</p>
          ...
          return (
            <button key={i} onClick={() => !futuro && onChange(i, anio)}
              style={{
                ...
                fontSize: 12, fontWeight: activo ? 700 : 500, opacity: futuro ? 0.3 : 1,
